...
\caption{Sequencing summary statistics for libraries.  Protocols are the
      shorthands used in table \ref{tab:protocolcosts}.  Reads indicates the
      total number of reads, and Mapped the total number of reads that mapped at
      least once to either genome.  Experiments 2 and 3 were run in a single
      HiSeq lane each.  Yield estimates were generated by adjusting Qubit High
      Sensitivity DNA readings by the average fragment size as measured by
      Bioanalyzer.}
\begin{center}
\begin{tabular}{|c|c|c|c|c|c|} \hline
      Expt & Protocol & \% {\em D. virilis} &   Yield & Total Reads & Mapped Reads \\\hline ''')
...
for i, row in config_file.iterrows():
    ...
    print((r"{expt} & {prot} & {frac}\% & {libyield:,} fmole & {total:,}"
          r" &{mapped:,} ({pct:.0%})\\").format(prot = protocol_map[protocol],
